Shader "Hidden/LG/Post/Depth of Field"
{
	/*
	...
	Properties
	{
		_FocusDistance("Focus Distance", Float) = 1
		_FocusWidth("Focus Width", Float) = 0.5

		_Blur("Blur Amount (Values outside [0; 1] will cause artifacts)", Float) = 1

		[Toggle(DEBUG_DEPTH)] _DebugDepth("Debug Depth", Float) = 0
		[Toggle(DEBUG_FOCUS)] _DebugFocus("Debug Focus", Float) = 0
	}
	SubShader
	{
		/*
		...
		Pass
		{
			...
			#pragma multi_compile __ DEBUG_DEPTH
			#pragma multi_compile ___ DEBUG_FOCUS
			...
			float4 frag(V2F IN) : SV_Target
			{
				...
				float depth = LinearEyeDepth(UNITY_SAMPLE_TEX2D(_CameraDepthTexture, IN.uv0));

				#ifdef DEBUG_DEPTH
				//	Draw the focus debug instead of the final color
				return depth.rrrr;
				#endif

				//	Calculate the focus
				float bluriness = smoothstep(0, 1, abs(depth - _FocusDistance) / _FocusWidth);

				#ifdef DEBUG_FOCUS
				//	Draw the focus debug instead of the final color
				return (1 - bluriness).rrrr;
				#endif

				//	Blend the blur color onto the screen color
				return lerp(screenColor, blurColor, bluriness);
			}
			ENDHLSL
		}
	}
