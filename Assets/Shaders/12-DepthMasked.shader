Shader "LG/12 Depth Masked"
{
	...
	V2F vert(MeshData meshData)
	{
		...
		#ifdef ENF_ORTHO
		//...	Make sure that the resulting TBN space is orthonormal
	...
	{
		/*
		 * This shader is basically a copy-paste of the vertex manipualtion shader.
		 * The render queue is the only difference. It's worth noting that it's not
		 * mandatory to make an additional shader just for this, as Unity allows,
		 * both via inspector and via code, to change the queue for a material.
		 * 
		 * Here we placed this object later in the render queue for showcasing the
		 * effect without manual configuration, but all what is needed is to make
		 * the masked geometry render after the depth mask shader's queue.
		 */
